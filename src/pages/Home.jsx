        ...
        <StaggerGroup className="relative mx-auto mt-14 grid max-w-5xl grid-cols-2 gap-4 px-6 sm:grid-cols-5">
          {quickTiles.map((tile) => (
            <StaggerItem key={tile.label}>
              <Link
                to={tile.to}
                className="group relative flex h-full flex-col items-center gap-3.5 overflow-hidden rounded-2xl border border-slate-200 bg-white px-4 py-7 text-center transition duration-300 hover:-translate-y-1 hover:border-brand-400/30 hover:shadow-xl hover:shadow-brand-500/10"
              >
                <span className="pointer-events-none absolute inset-x-0 -top-1/2 h-full scale-0 rounded-full bg-brand-400/10 blur-2xl transition duration-500 group-hover:scale-100" />
                <span className="relative flex h-12 w-12 items-center justify-center rounded-xl bg-gradient-to-br from-brand-400/20 to-brand-600/20 text-brand-500 transition duration-300 group-hover:scale-110 group-hover:rotate-3 group-hover:from-brand-400 group-hover:to-brand-600 group-hover:text-white">
                  <tile.icon size={22} />
                </span>
                <span className="relative text-xs font-semibold text-slate-700 group-hover:text-slate-900 sm:text-sm">{tile.label}</span>
              </Link>
            </StaggerItem>
          ))}
        </StaggerGroup>
